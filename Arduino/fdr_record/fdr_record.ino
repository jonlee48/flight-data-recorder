...
#define BUTTON 10
#define RECORD_LED 6 // I wired this wrong, uses inverted logic
#define RECORD_SIGNAL A5
#define STATUS_LED 8
#define RECEIVE_LED 13
#define RECEIVE_STATUS A2
...
void setup(void)
{
  ...
  pinMode(RECORD_SIGNAL, OUTPUT); // signal to sensor to start/stop recording
  pinMode(RECEIVE_STATUS, INPUT); // Red onboard LED lit when sensor is calibrated
  pinMode(BUTTON, INPUT_PULLDOWN); // button to start/stop record

  digitalWrite(RECORD_LED, HIGH); // blue LED was wired wrong, has inverted logic
...
void loop(void) {
  if (digitalRead(RECEIVE_STATUS) == HIGH)
  ...
  if (digitalRead(BUTTON) == HIGH && millis() >= record_last + debounce) {
    record_last = millis();
    record = !record;
    if (record) {
      Serial.println("start recording");
      digitalWrite(RECORD_LED, LOW);
      digitalWrite(RECORD_SIGNAL, HIGH);
      count = -1;
      ...
      digitalWrite(RECORD_SIGNAL, LOW);
    }
  }
